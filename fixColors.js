...
$.fn.fixColors = function(options) {
    ...
    var htmlcolorMap = {
        ...
        function colorNormalize(colorValue) {
            if (colorValue === null || typeof (colorValue) !== 'string') {
                return "rgba(0,0,0,0)";
            }
            colorValue = colorValue.toLowerCase();
            if (colorValue.indexOf('transparent') >= 0) {
                return "rgba(0,0,0,0)";
            }
            if (colorValue.match(/rgb.+[0-9]+/) === null) {
                var hex = colorValue;
                ...
                if (hex.length <= 4) {
                    hex = hex.replace(/([0-9a-f])/g, '0' + '$1');
                }
                var rgb = [];
                hex = hex.replace(/[0-9a-f]{2}/g, function (m, n) {
                    rgb[rgb.length] = m;
                });
                var ret = 'rgb({r},{g},{b})';
                ret = ret.replace('{r}', parseInt(rgb[0], 16));
                ret = ret.replace('{g}', parseInt(rgb[1], 16));
                ret = ret.replace('{b}', parseInt(rgb[2], 16));
                return ret;
            }
            return colorValue;
        }
    ...
    function getColorInfo(strColor) {
        ...
        function getComputedColor(el, what) {
            ...
            var str = '';
            try {
                str = getStyle(el, w[what].css);
                if (str === undefined && w[what].css === 'background-color') {
                    str = getStyle(el, 'background');
                }
                str = colorNormalize(str);
            } catch (e) {
                str = w[what].v;
            }
            return (getColorInfo(str).isTransparent) ? getComputedColor(el.parentNode, what) : str;
        }
        /***********************************************
        ...
        ***********************************************/
        function fixColors(Node, pBg, pColor) {
            ...
            var child = null,
                contrast = Math.abs(color.brightness - bg.brightness);
            (contrast < 80) && $(Node).css('color', bg.maxContrast);
